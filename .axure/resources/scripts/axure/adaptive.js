$axure.internal(function ($ax) {
      ...
      $axure("*").each(function (diagramObject, elementId) {
        ...
        if (
          ...
          if (diagramObject.generateCompound) {
            ...
            $ax.style.applyImage(
              elementId,
              _getImageWithTag(images, "normal~"),
              "normal",
            );
          } else {
            if ($ax.style.IsWidgetDisabled(elementId)) {
              var disabledImage = _matchImage(
                elementId,
                images,
                [],
                "disabled",
  ...
  var _matchImage = function (id, images, viewIdChain, state, doNotProgress) {
    ...
    if (
      (state == "disabled" && $ax.style.IsWidgetSelected(id)) ||
      (state == "selected" && $ax.style.IsWidgetDisabled(id))
    ) {
    ...
    if (message == "switchAdaptiveView") {
      ...
      if (data.scale != 0) {
        ...
        var scaleN = (newScaleN =
          (MOBILE_DEVICE ? mobileWidth : $(window).width()) / bodyWidth);

        if (data.scale == 2) {
          var pageSize = $ax.public.fn.getPageSize();
          var hScaleN =
            (MOBILE_DEVICE ? data.viewportHeight : $(window).height()) /
      ...
      if (data.device) {
        ...
        if (!MOBILE_DEVICE) {
          _addNiceScroll(
            $("html"),
            { emulatetouch: true, horizrailenabled: false },
            true,
          );
