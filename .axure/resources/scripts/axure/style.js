$axure.internal(function ($ax) {
  ...
  $ax.style.SetWidgetHover = function (id, value) {
    ...
    var state = _generateMouseState(
      id,
      valToSet ? MOUSE_OVER : NORMAL,
      override == SELECTED,
    );
  ...
  $ax.style.ObjHasMouseDown = function (id) {
    var obj = $obj(id);
    if (
      $ax.style.getElementImageOverride(id, "mouseDown") ||
      (obj.style && obj.style.stateStyles && obj.style.stateStyles.mouseDown)
    )
      return true;
    ...
    for (var i = 0; i < chain.length; i++) {
      ...
      if (style && style.stateStyles && style.stateStyles.mouseDown)
        return true;
    }
    return false;
  };
  ...
  $ax.style.SetWidgetMouseDown = function (id, value, checkMouseOver) {
    ...
    var mouseState = _generateMouseState(
      id,
      state,
      $ax.style.IsWidgetSelected(id),
    );
  ...
  var _generateMouseState = function (id, mouseState, selected) {
    var isSelectedFocused = function (state) {
      ...
      var jObj = $("#" + id);
      if (state == SELECTED)
        return jObj.hasClass(FOCUSED) ? SELECTED_FOCUSED : state;
      else
        return jObj.hasClass(FOCUSED) || jObj.hasClass(SELECTED_FOCUSED)
          ? FOCUSED
          : state;
    };

    if (selected) {
      if (_style.getElementImageOverride(id, SELECTED))
        return isSelectedFocused(SELECTED);

      var obj = $obj(id);
      //var viewChain = $ax.adaptive.getAdaptiveIdChain($ax.adaptive.currentViewId);
      var viewChain = $ax.style.getViewIdChain(
        $ax.adaptive.currentViewId,
        id,
        obj,
      );
      viewChain[viewChain.length] = "";
      if ($ax.IsDynamicPanel(obj.type) || $ax.IsLayer(obj.type))
        return isSelectedFocused(SELECTED);
      ...
      for (var i = 0; i < viewChain.length; i++) {
        ...
        if (
          (obj.adaptiveStyles &&
            obj.adaptiveStyles[viewId] &&
            any(obj.adaptiveStyles[viewId])) ||
          (obj.images &&
            (obj.images[scriptId + "~selected~" + viewId] ||
              obj.images["selected~" + viewId]))
        )
          return isSelectedFocused(SELECTED);
      }
      var selectedStyle =
        obj.style && obj.style.stateStyles && obj.style.stateStyles.selected;
      if (selectedStyle && any(selectedStyle))
        return isSelectedFocused(SELECTED);
    }
  ...
  $ax.style.SetWidgetFocused = function (id, value) {
    ...
    if (value)
      var state = $ax.style.IsWidgetSelected(id) ? SELECTED_FOCUSED : FOCUSED;
  ...
  var _generateSelectedState = function (id, selected) {
    var mouseState =
      $ax.event.mouseDownObjectId == id
        ? MOUSE_DOWN
        : $.inArray(id, $ax.event.mouseOverIds) != -1
          ? MOUSE_OVER
          : NORMAL;
  ...
  var _generateState = (_style.generateState = function (id) {
    return $ax.placeholderManager.isActive(id)
      ? HINT
      : _style.IsWidgetDisabled(id)
        ? DISABLED
  ...
  let _updateStateClassesHelper = function (
    ...
    if (event == DISABLED || event == SELECTED) {
      ...
      if (
        diagramObject &&
        $ax.public.fn.IsSelectionButton(diagramObject.type)
      ) {
    ...
    if (addMouseOverOnMouseDown && event == MOUSE_DOWN)
