...
$axure.internal(function ($ax) {
  ...
  $ax.public.fn.SetPanelState = function (stateNumber, options, showWhenSet) {
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      if ($ax.public.fn.IsDynamicPanel($ax.getTypeFromElementId(elementId))) {
        var stateName = $ax.visibility.GetPanelStateId(
          elementId,
        ...
        if (
          ...
          $ax.dynamicPanelManager.compressDelta(
            elementId,
            $ax.visibility.GetPanelState(elementId),
            stateName,
            options.vertical,
    ...
    for (var index = 0; index < elementIds.length; index++) {
      ...
      if (options && options.showType == "lightbox") {
        ...
        if (lightbox.length == 0) {
          ...
          var parents = $ax("#" + elementId).getParents(true, [
            "dynamicPanel",
          ])[0];
  ...
  var _setVisibility = function (elementId, value, options, useHide) {
    ...
    var wasShown = $ax.visibility.IsIdVisible(elementId);
    var compress =
      options && options.showType == "compress" && wasShown != value;
    if (compress)
      $ax.dynamicPanelManager.compressToggle(
        elementId,
        options.vertical,
