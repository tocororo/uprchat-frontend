...
$axure.internal(function ($ax) {
  ...
  var _refreshRepeater = function (
    ...
    if (itemsPregen) {
      ...
      for (var pos = start; pos < end; pos++) {
        ...
        for (var i = 0; i < templateIds.length; i++)
          $ax.initializeObjectEvents(
            $ax("#" + _createElementId(templateIds[i], itemId)),
            refreshType,
          );
  ...
  var applyFilter = function (repeaterId, data, eventInfo) {
    ...
    if (filters.length != 0) {
      ...
      outer: for (var i = 1; i <= data.length; i++) {
        for (var j = 0; j < filters.length; j++) {
          ...
          if ($ax.expr.evaluateExpr(filters[j].filter, eventInfo) != "true")
            continue outer;
        }
