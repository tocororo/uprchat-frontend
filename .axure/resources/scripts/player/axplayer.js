...
(function () {
  ...
  function contentDocument_onload() {
    (function setRepositionWhenReady() {
      ...
      if (
        ...
        var $iframe = $(
          $("#mainPanel").find("iframe")[0].contentWindow.document,
        );
  ...
  function repositionClippingBoundsScroll() {
    ...
    (function repositionWhenReady() {
      if (
        $($("#mainPanel").find("iframe")[0].contentWindow.document.body)
          .length === 0
      ) {
  ...
  var feedbackServiceUrl =
    (window.AXSHARE_HOST_SECURE_URL || "https://share.axure.com") + "/issue";
      ...
      if (safariAuthResponseProfile)
        userAcct.userName =
          response.username == null || response.username.trim() === ""
            ? response.userEmail
            : getUserName(response.username.trim());
      else
        userAcct.userName =
          response.nickname == null || response.nickname.trim() === ""
            ? response.userEmail
            : getUserName(response.nickname.trim());
      ...
      if (data.clipToView) {
        ...
        if (scaleVal == "2" || (!MOBILE_DEVICE && scaleVal == "0")) {
          var scaleN = (newScaleN =
            $("#mainPanel").width() / data.viewportWidth);
  ...
  function setUpController() {
    ...
    var _getLinkUrl = function (baseUrl) {
      ...
      return toAdd.length > 0
        ? baseUrl + (_shouldSendVarsToServer() ? "?" : "#") + toAdd + "&CSUM=1"
        : baseUrl;
    };
    ...
    _player.createPluginHost = function (settings) {
      if (
        !settings.context ||
        !(settings.context === "project" || settings.context === "inspect")
      ) {
